...
const Api = () =>  { 
  ...
  const fatchAPI = useCallback(async () => {
    try {
      ...
      //   setNotFound(res.data.hits.length === 0); // Check if results are empty
    } catch (error) {
      setIsLoading(false);
  ...
  return (
    <>
      <div className="bg-slate-800 flex justify-between items-center w-[100%]  fixed top-0 pt-1 pb-1 pl-5 pr-5">
        ...
        {/* <button className="border-2 border-slate-300 text-slate-300 pt-1 pl-4 pr-4 font-semibold rounded pb-1">SignUp</button> */}
      </div>
      <div className="">
        {isLoading ? (
          ...
          <div>
            <div className="text-center  mb-8 mt-24">
              <div className="flex mt-6 justify-center">
                <p className="text-slate-800 tracking-tight text-4xl font-bold">Choose Your Favorites Pokemon</p>
              </div>
              ...
              {/* {!notfound && (
                ...
                {/* <div className="mt-2">
                    <button
                      onClick={() => setPage(Page - 1)}
                      className=" bg-slate-800 pl-6 pr-6 pt-3 pb-3 mr-10 rounded text-white font-semibold cursor-pointer"
                    >
                      Previous
                    </button>
                    <button
                      onClick={() => setPage(Page + 1)}
                      className="mr-4 bg-slate-800 pl-6 pr-6 pt-4 pb-3 ml-10 rounded text-white font-semibold cursor-pointer "
                    >
                      Next
                    </button>
                </div> */}
            </div>
            <button onClick={GotoTop} className="bg-slate-800 bottom-16  left-1 rounded-full p-1 sticky transform -rotate-45"><BiSolidRocket className="text-3xl text-slate-300  rounded-full" /></button>
            <footer className="bg-slate-800 h-40 mt-10 pb-5 pt-2 flex-wrap  w-full flex justify-between pl-5 pr-5 items-center absolute">
             ...
             <div className="text-slate-300 font-medium"> Pokemons Provides You Diffrents Pokemons Which Can Help You <br /> To Do Your Work Professionally </div>
              <div>
                ...
                <form className="flex justify-center items-center" action="https://formspree.io/f/mleqnbww" method="POST">
                  ...
                  <button className=" pt-3 pb-3 pl-2 pr-2 rounded-r border-l-2 border-l-slate-800 bg-slate-300 text-slate-800 font-semibold"><BsSendFill className="object-cover" /></button>
                </form>
              </div>
            </footer>
          </div>
